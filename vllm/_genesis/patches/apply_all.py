...
@register_patch("PN98 aviso de dimensionamiento de L2")
def apply_patch_N98_l2_sizing_advisory() -> PatchResult:
    """PN98: loguea el tamano de L2 en tokens y el minimo recomendado."""
    name = "PN98 aviso de dimensionamiento de L2"
    if not _APPLY_MODE:
        return _applied(name, "dry-run: text-patch ready")
    try:
        from vllm._genesis.wiring.hybrid import patch_N98_l2_sizing_advisory
    except Exception as e:
        return _failed(name, f"wiring import failed: {e}")
    status, reason = patch_N98_l2_sizing_advisory.apply()
    if status == "applied":
        return _applied(name, reason)
    if status == "skipped":
        return _skipped(name, reason)
...
@register_patch("PN102 desalojo por score de bloque con envejecimiento")
def apply_patch_N102_block_score_eviction() -> PatchResult:
    """PN102: la victima de desalojo se elige por score, no por posicion."""
    name = "PN102 desalojo por score"
    if not _APPLY_MODE:
        return _applied(name, "dry-run: text-patch ready")
    try:
        from vllm._genesis.wiring.hybrid import patch_N102_block_score_eviction
    except Exception as e:
        return _failed(name, f"wiring import failed: {e}")
    status, reason = patch_N102_block_score_eviction.apply()
    if status == "applied":
        return _applied(name, reason)
    if status == "skipped":
        return _skipped(name, reason)
