...
| PN100 | Anillo de staging por clase de agente: los bloques de agentes **efímeros** salen de K bloques reservados al final de L2, así que no pueden desalojar el prefijo del hilo principal. El **preámbulo compartido** de un subagente (misma clave de contenido en cada invocación) se reconoce y va al caché, no al anillo — sin esa distinción el anillo recicla justo lo más valioso. Medido: con L2 post-PN99 el baseline ya aguanta una ráfaga de 20 subagentes sin desalojar al principal (99,5% cacheado), así que el anillo es para L2 chica | Sander | ON (hooks); **inerte** con `GENESIS_PN100_RING_BLOCKS=0` | `GENESIS_ENABLE_PN100_STAGING_RING` (kill switch `GENESIS_DISABLE_PN100=1`) |
